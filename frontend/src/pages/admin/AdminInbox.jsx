...
import { inboxAPI, businessChatAPI } from '../../services/api';
import { EmptyState, ErrorState, SkeletonList } from '../../components/shared/AsyncState';
...
export default function AdminInbox() {
  ...
  // Two independent conversation systems feed this inbox: the unified
  // multi-channel inbox (email/SMS/WhatsApp/in-app, keyed to booking
  // customers) and the in-app chat a signed-in consumer starts from a
  // business's public profile (keyed to their account, not a booking
  // customer record). Without merging both, messages sent through the
  // profile "Message business" button were invisible here.
  const refresh = () => {
    setFailed(false);
    return Promise.all([
      inboxAPI.conversations().catch(() => []),
      businessChatAPI.getRooms().catch(() => []),
    ]).then(([inboxRooms, chatRooms]) => {
      const fromInbox = inboxRooms.map(r => ({ ...r, source: 'inbox' }));
      const fromChat = chatRooms
        .filter(r => r.type === 'business_customer')
        .map(r => ({
          id: r.id,
          source: 'chat',
          customer_name: r.consumer_name,
          customer_email: r.consumer_email,
          last_message_preview: r.last_message,
          last_message_at: r.last_message_at,
          upcoming_booking: null,
          unread_count: 0,
        }));
      setRooms([...fromInbox, ...fromChat].sort((a, b) => new Date(b.last_message_at || 0) - new Date(a.last_message_at || 0)));
    }).catch(e => { setFailed(true); toast.error(e.message || 'Could not load inbox'); }).finally(() => setLoading(false));
  };
  ...
  const open = async id => {
    ...
    try {
      if (room?.source === 'chat') {
        const messages = await businessChatAPI.getMessages(id);
        setDetail({
          source: 'chat',
          conversation: { full_name: room.customer_name, email: room.customer_email, phone: null },
          upcoming_booking: null,
          previous_bookings: [],
          messages: messages.map(m => ({
            id: m.id, content: m.content, channel: 'in_app', status: 'sent', created_at: m.created_at,
            direction: m.sender_type === 'business' ? 'outbound' : 'inbound',
          })),
        });
      } else {
        setDetail({ source: 'inbox', ...(await inboxAPI.detail(id)) });
      }
    } catch (e) { toast.error(e.message || 'Could not load conversation'); }
  };
  ...
  const send = async e => {
    ...
    try {
      if (detail.source === 'chat') {
        const m = await businessChatAPI.sendMessage(active, text);
        setDetail(d => ({ ...d, messages: [...d.messages, { id: m.id, content: m.content, channel: 'in_app', status: 'sent', created_at: m.created_at, direction: 'outbound' }] }));
      } else {
        const m = await inboxAPI.send(active, { content: text, channel });
        setDetail(d => ({ ...d, messages: [...d.messages, m] }));
        if (m.status === 'queued') toast('Queued — this provider is not configured.');
      }
      setText('');
      refresh();
    } catch (e) { toast.error(e.message || 'Could not send message'); }
  };
