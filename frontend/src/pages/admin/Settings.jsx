          ...
          {business && (
            <>
              ...
              <div className="flex gap-2">
                <button type="button" onClick={() => navigate(`/book/${business.slug}`)} className="btn-primary flex-1 justify-center">
                  Open Booking Page
                </button>
                <button
                  type="button"
                  onClick={() => openExternalUrl(`https://wa.me/?text=${encodeURIComponent(`Book an appointment with ${business.name}: ${bookingUrl}`)}`).catch(() => toast.error('Could not open WhatsApp'))}
                    ...
                    <form onSubmit={submitVerification} className="space-y-4">
                      ...
                      {!verForm.sole_trader && (
                        <div>
                          ...
                          <p className="text-xs text-gray-400 mt-1">Find yours at <a href="https://find-and-update.company-information.service.gov.uk" target="_blank" rel="noopener" onClick={(event) => { event.preventDefault(); openExternalUrl('https://find-and-update.company-information.service.gov.uk').catch(() => toast.error('Could not open Companies House')); }} className="text-primary-600 underline">Companies House</a></p>
                        </div>
                      )}
                      <div>
