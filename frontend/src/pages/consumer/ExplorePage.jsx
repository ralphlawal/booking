...
function StarRating({ rating }) {
  ...
  return (
    <span className="flex items-center gap-0.5 text-amber-400 text-xs">
      ...
      <span className="text-gray-500 ml-1">{r > 0 ? r.toFixed(1) : 'New'}</span>
    </span>
  );
}
    ...
    >
      <div className="h-28 bg-gradient-to-br from-primary-100 to-primary-200 dark:from-primary-900/40 dark:to-primary-800/40 flex items-center justify-center relative">
        ...
        {biz.distance_km !== null && biz.distance_km !== undefined && (
          <span className="absolute top-2 right-2 bg-white/90 dark:bg-gray-900/90 text-xs font-semibold px-2 py-1 rounded-full text-gray-700 dark:text-gray-200">
            📍 {biz.distance_km} km
          </span>
        )}
      </div>
      <div className="p-4 flex flex-col gap-1 flex-1">
        ...
        {biz.location && (
          <p className="text-xs text-gray-400 line-clamp-1">📍 {biz.location}</p>
        )}
        {biz.description && (
        ...
        <div className="mt-auto pt-3 flex items-center justify-between">
          ...
          <span className="text-xs text-primary-600 dark:text-primary-400 font-semibold group-hover:underline">
            Book →
          </span>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
      ...
      <nav className="sticky top-0 z-50 bg-white/90 dark:bg-gray-900/90 backdrop-blur border-b border-gray-100 dark:border-gray-800">
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <Link to="/match" className="text-sm font-semibold text-primary-600 dark:text-primary-400 hidden sm:block">
              ⚡ Smart Match
            </Link>
            {consumer ? (
      ...
      <div className="bg-gradient-to-b from-primary-700 to-primary-900 px-4 py-10 text-center">
        ...
        <form onSubmit={handleSearch} className="max-w-lg mx-auto flex gap-2">
          <input
            className="flex-1 px-4 py-3 rounded-xl text-sm outline-none border-0 shadow-sm"
          ...
          />
          <button type="submit" className="bg-white text-primary-700 font-bold px-5 py-3 rounded-xl text-sm hover:bg-gray-50 transition-colors">
            Search
          </button>
        </form>
        <button
