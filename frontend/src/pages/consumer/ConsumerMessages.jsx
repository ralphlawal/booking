...
import { MessageSquare, ArrowLeft, Headphones } from 'lucide-react';
...
function RoomRow({ room, active, onClick }) {
  ...
  return (
    ...
    >
      <div className={`w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 text-sm font-bold ${isSupport ? 'bg-indigo-100 dark:bg-indigo-900/40 text-indigo-600' : 'bg-primary-100 dark:bg-primary-900/40 text-primary-600 dark:text-primary-400'}`}>
        {isSupport ? <Headphones className="w-4 h-4" /> : (label[0] || '?').toUpperCase()}
      </div>
...
export default function ConsumerMessages() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex flex-col">
      ...
      <div className="flex-1 max-w-4xl mx-auto w-full px-4 py-4 pb-24 flex gap-4" style={{ height: 'calc(100vh - 9rem)' }}>
        {/* Room list */}
        <div className="w-64 flex-shrink-0 card p-0 overflow-hidden flex flex-col h-full">
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              <div className="p-4 space-y-3">
                {[...Array(3)].map((_, i) => <div key={i} className="h-12 bg-gray-100 dark:bg-gray-800 rounded-xl animate-pulse" />)}
              </div>
            ) : rooms.length === 0 ? (
          ...
          <div className="p-3 border-t border-gray-100 dark:border-gray-800">
            <button
              onClick={openSupport}
              className="w-full flex items-center justify-center gap-2 text-xs font-semibold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/20 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 rounded-xl py-2 transition-colors"
            >
